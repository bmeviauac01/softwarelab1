...
Írj tárolt függvényt `CheckInvoice` néven, aminek bemeneti paramétere egy `int` típusú és `@invoiceid` nevű számlaazonosító.

- Az függvény ellenőrizze, hogy a paraméterben kapott számlához (`Invoice`) kapcsolódó számlatételeken (`InvoiceItem`) szereplő mennyiség (`Amount`) egyezik-e a kapcsolódó megrendeléstétel (`OrderItem`) mennyiségével. (Az `InvoiceItem` rekord hivatkozik a kapcsolódó `OrderItem`-re.)
...
- Csak akkor írjon bármit a kimenetre a tárolt függvény, ha problémát talált. Semmiképpen se hagyj teszteléshez használt kiírást az függvényben!
- Legyen az függvény `int` típusú visszatérési értéke 0, ha nem kellett semmit kiírni a kimenetre, és 1, ha kellett. Ez az érték `return` kulcsszóval kerüljön visszaadásra (ne `output` paraméter legyen).
...
!!! example "BEADANDÓ"
    Az függvény kódját az `f2-procedure.sql` fájlba írd. A fájlban csak ez az egyetlen `create proc` utasítás legyen! A feladat helyes megoldása 4 pontot ér. Helyesség függvényében részpontszám is kapható.

### Minden számla ellenőrzése

Írj T-SQL kódblokkot, ami az előző feladatban megírt függvényt hívja meg egyenként az összes számlára. Érdemes ehhez egy kurzort használnod, ami a számlákon fut végig.
...
!!! tip "Tárolt függvény meghívása"
    Tárolt függvény meghívása kódból az `exec` paranccsal lehetséges, pl.
...
!!! example "BEADANDÓ"
    Az összes számlát ellenőrző kódot az `f2-check-all.sql` fájlba írd. A fájlban csak a lefuttatható kódblokk szerepeljen. Ne legyen benne a tárolt függvény kódja, ne legyen benne se `[use]` se `go` utasítás. A feladatrésszel 2 pont szerezhető.
